            ...
            <div class="ibox-content">
                <div class="table-responsive">
                    <form method="GET">
                        ...
                        <span>Show </span>
                        <select class="select2 col-sm-1" name="sortir" placehorder={{$sortir}} id="sortir" autofocus="true" value="{{$sortir}}" >
                    ...
                    <div>
                        @php
                    $from = 1 + (($pemilikbisnis -> currentPage() - 1) * $pemilikbisnis -> perPage());
                    $to = $from + $sortir -1;
                    $jumlahdata = 1 + ( $pemilikbisnis -> lastPage() * $pemilikbisnis -> count()) -1;
                        @endphp
                        <span class="sm-12">Showing {{$from}} to {{$to}} of {{$jumlahdata}} entries</span>

                    </div>
                    <table class="table table-striped table-bordered table-hover" id="pemilikBisnis" >
                        <thead>
                            <tr>
